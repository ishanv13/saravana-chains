export default function WhatsAppFAB() {
  return (
    <a
      href="https://wa.me/+919944442901"
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Chat on WhatsApp"
      className="wa-fab"
    >
      <svg viewBox="0 0 24 24" fill="white" width="26" height="26" aria-hidden="true">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
        <path d="M12 0C5.373 0 0 5.373 0 12c0 2.124.553 4.118 1.522 5.853L.057 23.57a.5.5 0 0 0 .614.614l5.7-1.462A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.907 0-3.692-.51-5.228-1.398l-.374-.222-3.884.997.997-3.885-.222-.374A9.955 9.955 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z" />
      </svg>
      <style>{`
        .wa-fab {
          position: fixed; bottom: 22px; right: 22px; z-index: 70;
          width: 54px; height: 54px; border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          background: #1faa54; text-decoration: none;
          box-shadow: 0 6px 22px rgba(0,0,0,0.45), 0 0 0 1px rgba(240,212,136,0.25);
          opacity: 0; transform: scale(.6);
          animation: waIn .5s cubic-bezier(.16,1,.3,1) 1.6s forwards;
          transition: transform .3s, box-shadow .3s;
        }
        .wa-fab:hover { transform: scale(1.08); box-shadow: 0 8px 28px rgba(0,0,0,0.5), 0 0 0 1px rgba(240,212,136,0.5); }
        @keyframes waIn { to { opacity: 1; transform: scale(1); } }
        @media (prefers-reduced-motion: reduce) { .wa-fab { opacity: 1; transform: none; animation: none; } }
      `}</style>
    </a>
  )
}
